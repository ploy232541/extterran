...
class LearnScreen extends Component {
  ...
  async componentDidMount() {
    try {
        ...
        .then(async (response) => {
          ...
          if (result != null) {
            for (i in result) {
              value = result[i];
              this.setState({
                image_course: value.course_picture,
      ...
      httpClient
        .get(
          `/CourseOnline/getAlertModelDateTimeCourse/${course_id}/${user_id}`
        )
        .then(async (response) => {
          const result = response.data;
          if (result != null) {
        ...
        .then(async (response) => {
          ...
          if (result != null) {
            this.setState({
              ...
              checkHaveCer: result.checkHaveCer,
              statePrintCert: result.statePrintCert,
              course_workshop: result.course_workshop
            });
          }
        })
        .catch((error) => {
  ...
  PrintCertificate = async () => {
    ...
    Linking.openURL(
      `http://smartxlearning.com/CourseApiMobile/PrintCertificate?user_id=${user_id}&course_id=${course_id}&lang_id=${1}`
    );
  };
        ...
        >
          ...
          >
            <View style={{ marginTop: -30, marginBottom: -50 }}>
              ...
              >
                <Image
                  ...
                  source={{
                    uri:
                      this.state.image_course != null
                        ? this.state.image_course
            ...
            >
              ...
              {this.state.lang === "EN"
                ? "Remaining Time"
                : "ท่านมีระยะเวลาการเรียนคงเหลือ"}{" "}
              {this.state.diff} {this.state.lang === "EN" ? "Day" : "วัน"}
              {this.state.course_workshop == true
                ? this.state.lang === "EN"
                  ? " ( Workshop )"
                  : " ( มีปฏิบัติต่อ )"
                : null}
            ...
            {checkHaveCer == true ? (
              statePrintCert == true ? (
                <View>
                  ...
                  />
                  <TouchableOpacity onPress={this.PrintCertificate}>
                    <View
                      style={{
                        borderWidth: 1,
                        borderColor: "#bfbfbf",
                    ...
                    >
                      <Image
                        source={{
                          uri: "http://smartxlearning.com/themes/template/img/green-reward.png"
                        }}
                        style={{
                          width:
                            Platform.OS == "ios"
                      ...
                      />
                      <Text
                        style={{ fontSize: 12, marginTop: 0, marginBottom: 15 }}
                      >
                        {this.state.lang === "EN"
                          ? "Certificate Printed"
                    ...
                    onPress={() =>
                      Alert.alert(
                        this.state.lang === "EN"
                          ...
                          : "ท่านยังเรียนไม่ผ่านตามเงื่อนไข"
                      )
                    }
                  >
                    <View
                    ...
                    >
                      <Image
                        source={{
                          uri: "http://smartxlearning.com/themes/template/img/grey-reward.png"
                        }}
                        style={{ width: 80, height: 90, marginTop: 10 }}
                        resizeMode="contain"
                      />
                      <Text
                        style={{ fontSize: 12, marginTop: 0, marginBottom: 15 }}
