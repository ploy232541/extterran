        ...
        try {
          ...
          .then(async response => {
            const result = response.data;
              if (result != null) {
                this.setState({lesson_title: result.title, questions: result.questions, 
          ...
          <View style={styles.viewModal}>
            <View style={styles.itemStyle}>
                ...
                <Text numberOfLines={1} style={{fontSize: 16, fontWeight: '500', marginTop: 5, color: '#404040', marginLeft: 20, marginRight: 20}}>
                 {this.state.lang === "EN" ? 'Lesson Name : ' : 'ชื่อบทเรียน : '} {this.state.lesson_title}
                </Text>
                ...
                <View style={{alignSelf: 'center'}}>
                {
                  type == 'pre' ? 
                  <Button 
                    onPress={() => this.props.navigation.dispatch(StackActions.replace('PreTest', {lesson_id: lesson_id, type: type}))}
                  ...
                  :
                  <Button 
                    onPress={() => this.props.navigation.dispatch(StackActions.replace('PostTest', {lesson_id: lesson_id, type: type}))}
                    style={{height: 35, marginBottom: 20, borderRadius: 5, backgroundColor: '#ff8000'}}
                    >
                      <Text style={{marginRight: 10, marginLeft: 10, color: '#fff', fontWeight: 'bold'}}>{this.state.lang === "EN" ? 'Test' : 'ทำแบบทดสอบ'}</Text>
                  </Button> 
