    ...
    useEffect(() => {
        const run = async () => {
            try {
              ...
              httpClient
                .get(`/CourseOnline/getCategoryCourseGeneral/${user_id}/${lang_id}/${type}`)
                .then(response => {
    ...
    const renderButtonCard = ({ item }) => {
        ...
        return (
            <View style={{ flex: 1, margin: 5 }}>
                        <TouchableOpacity 
                        onPress={() => navigation.navigate("CousreScreenDetailGeneral", {cate_id: item.cate_id, title: item.cate_title, type: item.type})}
                        >
                        <Card>
